...
resource "google_storage_bucket_iam_policy" "bucket_pres2_public" {
  bucket      = google_storage_bucket.bucket_pres2.name
  policy_data = data.google_iam_policy.viewer.policy_data
}
...
resource "google_storage_bucket" "bucket_pres2" {
  project                     = var.project
  name                        = var.pres2
  force_destroy               = false
  location                    = "US"
...
resource "google_compute_backend_bucket" "pres2_backend" {
  project     = var.project
  name        = "backend-${var.pres2}"
  bucket_name = google_storage_bucket.bucket_pres2.name
  enable_cdn  = true
}
...
resource "google_compute_url_map" "default" {
  ...
  host_rule {
    hosts        = ["${var.pres2}.${var.host}"]
    path_matcher = var.pres2
  }
  ...
  path_matcher {
    name            = var.pres2
    default_service = google_compute_backend_bucket.pres2_backend.id
    path_rule {
      paths   = ["/*"]
      service = google_compute_backend_bucket.pres2_backend.id
    }
  }
